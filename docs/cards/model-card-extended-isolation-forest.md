...
Isolation‑Forest isolates a point by random splits; **shorter paths** imply **more anomalous** inputs. For a subsample size \(n\), the expected path length of an unsuccessful search is
\[
c(n) \,=\, 2H(n-1) - \frac{2(n-1)}{n}, \qquad
H(m) \,=\, \sum_{k=1}^{m} \frac{1}{k} \approx \ln(m) + \gamma.
\tag{1}
\]
Let \(h(x)\) be the average **path depth** of sample \(x\) across trees. A common anomaly score is
\[
s(x) \,=\, 2^{-\, h(x) / c(n)}, \quad \text{so}\; h \downarrow \Rightarrow s \uparrow. \tag{2}
\]
**isotree** returns **average depth** (larger = more normal). We therefore declare an anomaly if
\[
\text{hot}(x) \,=\, \mathbb{1}\big[\, h(x) < T \;\lor\; h(x) < T_{\text{ext}}\,\big], \tag{3}
\]
where:
...
- **PII**: ADC signals contain no PII; scrub any auxiliary metadata upstream.  
